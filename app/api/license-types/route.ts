...
export async function GET() {
  ...
  const rows = await prisma.licenseType.findMany({
    where: { companyId: auth.session.user.companyId },
    orderBy: { name: "asc" },
  });
  return NextResponse.json({ licenseTypes: rows });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const row = await prisma.licenseType.create({
      data: {
    ...
    return NextResponse.json({ licenseType: row }, { status: 201 });
  } catch (e) {
    if (
      e instanceof Prisma.PrismaClientKnownRequestError &&
      e.code === "P2002"
    ) {
      return badRequest("A license type with that name already exists.");
    }
    throw e;
  }
}
